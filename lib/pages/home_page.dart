...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
                ...
                SizedBox(
                  ...
                  child: FutureBuilder(
                    ...
                    builder: (context, snapshot) {
                      ...
                      List<int> tomorrowHourlyIndices =
                          _getTomorrowHourlyIndices(weather);
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Container(
                              ...
                              child: _buildPrecipitationChart(weather),
                            ),
                            SizedBox(height: 16),
                          ],
                        ),
  ...
  Widget _buildPrecipitationChart(Weather weather) {
    List<FlSpot> spots = [];
    List<String> labels = [];
    ...
    for (int i = 0; i < weather.hourly.time.length && i < 24; i++) {
      DateTime hourTime = DateTime.parse(weather.hourly.time[i]);
      if (hourTime.isAfter(now)) {
        double precipitation = weather.hourly.precipitation[i].toDouble();
        spots.add(FlSpot(spots.length.toDouble(), precipitation));
        if (precipitation > maxPrecipitation) {
          maxPrecipitation = precipitation;
        }
        if (i % 3 == 0) {
          labels.add(DateFormat("HH:mm").format(hourTime));
        } else {
          labels.add("");
        }
      }
    ...
    return LineChart(
      LineChartData(
        ...
        minY: 0,
        maxY: maxPrecipitation > 0 ? maxPrecipitation * 1.2 : 1,
      ),
    );
  }
